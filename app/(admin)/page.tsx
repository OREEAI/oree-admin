...
// Colors for the breakdown rows. Known org statuses get semantic colors;
// everything else (tiers, unknown labels) walks a brand-derived
// categorical ramp ranked by size, so the biggest segment is coral and
// the rest recede toward navy tints.
        ...
        <>
          ...
          <div className="mt-4 flex h-2.5 w-full gap-px overflow-hidden rounded-full bg-surface-soft">
            {rows.map(([name, count], i) => {
              const color = STATUS_COLORS[name.toLowerCase()] ?? RANKED_COLORS[i % RANKED_COLORS.length];
              return (
                <div
                  key={name}
                  title={`${name}: ${count}`}
